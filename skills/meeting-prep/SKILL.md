...
- **Open with a one-line identity header, before anything else:** the person's **name, current role, and company**, plus a one-sentence "who they are," and, when you have one, their **LinkedIn profile link right there in the header** (either link the name or add a "· LinkedIn" link at the end), so it's one tap from the top. Only include a LinkedIn URL you actually found via real search; never fabricate or guess one, and omit it if you don't have a verified link. This header is the very first line on the page: above "Why this meeting," above any "What's new" update, above the deep dive. The reader must know who they're meeting, and be able to open their LinkedIn, from the first line without scrolling.
...
For each prep event whose SOURCE meeting starts within the next 48 hours: do a quick recency check on the person/company (web search, last ~2 weeks: funding, product launches, role changes, notable posts/press). ALSO re-check `~~chat` DMs, and any other connected source (transcripts, CRM, docs), for new activity since the brief was written that changes the topic or adds agenda. If anything material is new: update the brief's **single** `## What's new (as of <date>)` section, placed directly BELOW the identity header, not above it, REPLACING the previous "What's new" rather than stacking a new block on top (stacked updates bury the identity and bloat the top of the page; keep just the latest, and fold anything still relevant from the prior one into it). The identity header stays the first thing on the page. Also refresh the Key links in the prep event description if relevant. Keep the `AUTOPREP` marker, event summary, and event time unchanged. (For `~~files` fallback briefs: create a fresh file with "WHAT'S NEW" on top and repoint the prep event link.) If nothing material is new, note "no change." Only ever modify prep briefs/events; never the source.
