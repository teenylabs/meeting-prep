...
- Part B: `get_event(source)`; if start/end differ, `update_event` on the PREP event only. **Re-pass `colorId` "8", `visibility` "private", and `overrideReminders: []` on the update:** an `update_event` that omits them can reset the event to the calendar's default color and reminders.
